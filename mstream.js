...
exports.serveIt = (config) => {
  ...
  if (program.users && Object.keys(program.users).length !== 0) {
    ...
    if (program["lastfm-user"] && program["lastfm-password"]) {
      ...
      program.users["mstream-user"]["lastfm-password"] =
        program["lastfm-password"];
    }
  ...
  Object.keys(program.folders).forEach((key) => {
    mstream.use(
      "/media/" + key + "/",
      express.static(program.folders[key].root)
    );
  });

  // Start the server!
  server.on("request", mstream);
  server.listen(process.env.PORT || program.port, () => {
    const protocol =
      program.ssl && program.ssl.cert && program.ssl.key ? "https" : "http";
    winston.info(
      `Access mStream locally: ${protocol}://localhost:${
        process.env.PORT || program.port
      }`
    );
    winston.info(
      `Try the WinAmp Demo: ${protocol}://localhost:${
        process.env.PORT || program.port
      }/winamp`
    );
